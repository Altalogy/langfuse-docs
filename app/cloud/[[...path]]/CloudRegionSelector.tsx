...
const getCloudRedirectParts = (pathname: string, search: string) => {
  const sanitizedPathname = stripControlChars(pathname || "");

  let cloudSubpath = "/";
  if (
    sanitizedPathname.startsWith(`${CLOUD_ROUTE_PREFIX}/`) &&
    sanitizedPathname.length > CLOUD_ROUTE_PREFIX.length
  ) {
    cloudSubpath = sanitizedPathname.slice(CLOUD_ROUTE_PREFIX.length);
  }

  return { cloudSubpath, search: search ? `?${search}` : "" };
};
...
function CloudRegionSelectorInner() {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const signedInRegions = useCloudRegionSignIn();

  const { cloudSubpath, search: fallbackSearch } = useMemo(
    () => getCloudRedirectParts(pathname, searchParams.toString()),
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <Button asChild size="sm" className="whitespace-nowrap">
                      <a
                        href={href}
                        onClick={(event) =>
                          handleRegionSelect(regionKey, event)
                        }
                      >
                        Continue
